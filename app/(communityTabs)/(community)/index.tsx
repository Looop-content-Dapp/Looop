...
import { useTribes, useUserSubscriptions } from '@/hooks/useTribes';
import PostCard from '@/components/cards/PostCard';
import { ArrowRight01Icon } from '@hugeicons/react-native';
import { TribeCardSkeleton, SubscriptionCardSkeleton } from '@/components/skeletons/TribeSkeletons';
import { TribeItem, SubscriptionItem } from '@/types/tribe';

const ArtistBadge = React.memo(({ artist }) => (
...
const MembersList = React.memo(({ members, memberCount }) => (
  <View className="flex-row items-center bg-[#111318] flex-1 rounded-[16px] p-[10px]">
    <View className="flex-row -space-x-2">
      {members.slice(0, 3).map((member) => (
        <Avatar
          key={member._id}
          source={{ uri: member.profileImage || "https://i.pinimg.com/564x/bc/7a/0c/bc7a0c399990de122f1b6e09d00e6c4c.jpg" }}
          size={24}
          rounded
...
// Update TribeCard component
const TribeCard = React.memo<{ item: TribeItem; loading: boolean }>(({ item, loading }) => {
  if (loading) {
  ...
  return (
    ...
    >
      ...
      <View className="absolute bottom-0 bg-[#202227] p-[16px] justify-end">
        <Text className="text-white text-[16px] font-PlusJakartaSansBold mb-1" numberOfLines={1}>
          {item.name}
        </Text>
        <Text className="text-[#D2D3D5] text-[14px] font-PlusJakartaSansRegular mb-3" numberOfLines={1}>
          {item.description}
        </Text>
        <View className='flex-row items-center justify-between'>
          <MembersList members={item.members} memberCount={item.memberCount} />
...
const SubscriptionCard = React.memo<{ item: SubscriptionItem; loading: boolean }>(({ item, loading }) => {
  if (loading) {
  ...
  return (
    <TouchableOpacity
      className="w-full h-[100px] my-[20px] rounded-[16px] overflow-hidden flex-row items-center"
    >
      <Image
      ...
      <View className="flex-1 px-4 py-3 justify-between">
        ...
        <View className="flex-row items-center gap-2 mt-2">
          ...
          >
            <Text className={`text-[12px] font-PlusJakartaSansMedium ${
              status === 'Active member' ? 'text-[#2DD881]' : 'text-[#FF4444]'
            }`}>
              {status}
            </Text>
          </View>
          <View className={`px-3 py-1 rounded-full ${isExpired ? 'bg-[#FF4444]/10' : 'bg-[#202227]'}`}>
            <Text className={`text-[12px] font-PlusJakartaSansMedium ${isExpired ? 'text-[#FF4444]' : 'text-white/60'}`}>
              {dueText}
            </Text>
          </View>
        </View>
...
const Index = () => {
  ...
  const { data: tribesData, isLoading: isTribesLoading, refetch: refetchTribes } = useTribes();
  const {
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    await (activeTab === 'My Tribes' ? refetchTribes() : refetchSubscriptions());
    setRefreshing(false);
  ...
  const currentData = isSubscriptionsTab ? subscriptionsData?.data : tribesData?.data;
  const isLoading = isSubscriptionsTab ? isSubscriptionsLoading : isTribesLoading;
  ...
  // Update renderHeader function in Index component
    ...
    return (
      <View className="py-4">
        <FlatList
          ...
          renderItem={({ item }) => <TribeCard item={item} loading={isLoading} />}
          ListEmptyComponent={() => (
            isLoading ? (
