import { useSearch } from "@/hooks/core/useSearch";
...
const SearchScreen = () => {
  ...
  const handleResultPress = (
    item: Post | Artist | Community,
    type: "post" | "artist" | "community"
  ) => {
    switch (type) {
      case "post":
      ...
      case "artist":
        ...
        router.navigate({
          ...
          params: {
            ...
            isFollowing: artist.followers?.includes(userdata?._id as string)
              ? "true"
              : "false",
      ...
      case "community":
        ...
        const isMember =
          community?.members?.some((member) => member._id === userdata?._id) ||
          false;
  ...
  const renderPostSection = (data: Post[]) => {
    ...
    return (
      <View className="mb-6">
        <Text className="text-[#9A9B9F] text-[20px] font-PlusJakartaSansMedium mb-4 px-4">
          Posts from your tribes
        </Text>
        <View>
                  ...
                  >
                    <View className="flex-1 justify-end p-4">
                      <View className="flex-row items-center gap-x-1">
                        <Text
                          numberOfLines={1}
                          className="text-[#f4f4f4] text-[14px] font-semibold"
                        >
                          {(item as Artist).name}
                        </Text>
                  ...
                  >
                    <View className="flex-1 justify-end p-6">
                      ...
                      <Text className="text-[#63656B] text-[14px] mt-1">
                        {formatNumber(
                          (item as Community).memberCount.toString()
                        )}{" "}
                        Members
                      </Text>
                    </View>
        ...
        <View className="">
          <FlatList
            ...
            renderItem={({ item }) => (
              ...
              >
                <Text className="text-[#f4f4f4] font-PlusJakartaSansMedium text-[16px]">
                  {item}
                </Text>
              </Pressable>
            )}
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          keyExtractor={(item, index) =>
            typeof item === "string" ? item : String(index)
          }
          ...
          ListHeaderComponent={
            recentSearches.length > 0 ? (
              <Text className="text-[#f4f4f4] text-lg mb-4">
                Recent Searches
              </Text>
            ) : null
          }
        ...
        >
          ...
          {renderSection<Artist>(
            "Artistes",
            results.filter((item): item is Artist => item.type === "artist"),
            "artist"
          )}
          {renderSection<Community>(
            "Bubbling Communities",
            results.filter(
              (item): item is Community => item.type === "community"
            ),
            "community"
          )}
