...
const profileSchema = yup.object({
  ...
  preferences: yup
    .object({
      ...
      currency: yup
        .string()
        .oneOf(["USD", "EUR", "GBP", "NGN", "GHS", "KES", "ZAR"] as const)
        .default("USD"),
      chain: yup
        .string()
        .oneOf(["XION", "STARKNET"] as const)
        .default("XION"),
      theme: yup
        .string()
      ...
      displayMode: yup
        .string()
        .oneOf(["compact", "comfortable"] as const)
        .default("comfortable"),
    })
    .default({
      favoriteGenres: [],
  ...
  const {
    ...
    defaultValues: {
      ...
      preferences: {
        ...
        favoriteGenres:
          userdata?.preferences?.favoriteGenres?.filter(
  ...
  const onSubmit = async (data: ProfileFormData) => {
    try {
      const formattedData = {
        ...
        preferences: {
          ...data.preferences,
          favoriteGenres: data.preferences.favoriteGenres.filter(
            (genre): genre is string => genre !== undefined
          ),
        },
      };
