...
const index = () => {
  ...
  const { worldwideTracks, locationTracks, isLoading } = useExplore(location?.region || 'NG');
  ...
  const topAlbumsData = [
    ...
    },
    {
      ...
      title: "Work of Art",
      artist: "Asake",
      imageUrl: "https://i.scdn.co/image/ab67616d0000b273b56b946d1020c4c9cc2a3829",
      explicit: true
    }
  ];

  const worldwideChartData = worldwideTracks.data?.map((track) => ({
    position: track.rank,
    title: track.title,
  ...
  const locationChartData = locationTracks.data?.map((track, index) => ({
    position: index + 1,
    title: track.title,
    artist: track.artist.name,
    imageUrl: track?.release?.artwork.high,
