...
const useMusicPlayer = () => {
  ...
  const { streamSong, retrieveUserId, getTracksFromId, saveAlbum, likeSong, getLikedSongs, getSavedAlbums } = useQuery();
  ...
  const handlePlayPause = useCallback((tracks: Track[], albumInfo: AlbumInfo) => {
  ...
  const handleTrackPress = useCallback((track: Track, index: number, albumInfo: AlbumInfo, tracks: Track[]) => {
    play(track, albumInfo, tracks);
  }, [play]);

  const loadAlbumData = useCallback(async (albumId: string, type: string) => {
...
const handleLike = useCallback(async (id: string, type: string) => {
    try {
        const userId = await retrieveUserId();
        if (!userId) return;
  ...
  return {
    ...
    addToQueue: useCallback((track: Track) => dispatch(addToQueue(track)), [dispatch]),
    next: useCallback(() => dispatch(playNextTrack()), [dispatch]),
    previous: useCallback(() => dispatch(playPreviousTrack()), [dispatch]),
    updatePlaylist: useCallback((tracks: Track[]) => dispatch(setPlaylist(tracks)), [dispatch]),
    getStoredTrack,
  };
};
