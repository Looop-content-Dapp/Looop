...
import { SentIcon, ArrowLeft01Icon, FavouriteIcon, Comment02Icon } from '@hugeicons/react-native';
...
const CommentsBottomSheet: React.FC<CommentsBottomSheetProps> = ({
  ...
  const [currentPage, setCurrentPage] = useState<'comments' | 'reply'>('comments');
    ...
    return (
      <View className="mb-4 px-4">
        <View className="flex-row mb-1">
          <View className="mr-3">
            <Avatar
              source={{ uri: item.user.profileImage || "https://i.pinimg.com/564x/bc/7a/0c/bc7a0c399990de122f1b6e09d00e6c4c.jpg" }}
              size={38}
              rounded
          ...
          <View className="flex-1">
            <View className="flex-row items-center mb-1 justify-between">
              <Text className="text-[16px] font-PlusJakartaSansBold text-[#FFFFFF] mr-1.5">{item.user.username}</Text>
              ...
              <Text className="text-[12px] text-[#63656B] font-PlusJakartaSansMedium">{formatTimestamp(item.timestamp)}</Text>
            </View>
            <Text className="text-[14px] text-[#FFFFFF] font-PlusJakartaSansRegular mb-2 leading-5">{item.text}</Text>
            <View className="flex-row items-center">
              <TouchableOpacity className="flex-row items-center gap-x-2 mr-4" onPress={() => handleLike(item.id)}>
                <FavouriteIcon size={20} color="#63656B" />
                <Text className="text-[14px] text-[#D2D3D5] font-PlusJakartaSansMedium">{item.likes}</Text>
              </TouchableOpacity>
              <TouchableOpacity className="mr-4 flex-row items-center gap-x-2" onPress={() => handleReply(item)}>
                <Comment02Icon size={20} color='#63656B' />
                <Text className="text-[14px] text-[#D2D3D5] font-PlusJakartaSansMedium">Reply</Text>
              </TouchableOpacity>
              {item.replies && item.replies.length > 0 && (
                <TouchableOpacity onPress={() => handleReply(item)}>
                  <Text className="text-xs text-gray-500">{item.replies.length} Replies</Text>
                </TouchableOpacity>
              )}
            </View>
          </View>
  ...
  const renderCommentInput = () => (
    <KeyboardAvoidingView
      ...
      className={`border-t border-gray-800  w-full ${isKeyboardVisible ? 'pb-0' : `pb-${insets.bottom}`}`}
    >
      <View className="flex-row items-center px-4 py-3">
        <Avatar
          ...
          size={32}
          rounded
          containerStyle={{ marginRight: 12, borderWidth: 1, borderColor: '#f4f4f4' }}
        />
        <View className="flex-1 flex-row items-center bg-[#202227] rounded-[12px] px-4 py-3 min-h-10">
          <TextInput
            ...
            placeholder={currentPage === 'reply' ? `Reply to ${replyToComment?.user?.username}...` : "Write a comment..."}
          ...
          />
          {commentText.trim() && (
            <TouchableOpacity className="absolute right-10 p-1" onPress={() => setCommentText('')}>
              <Text className="text-[#787A80] text-lg">×</Text>
            </TouchableOpacity>
          )}
          <TouchableOpacity
            className={`absolute right-2.5 p-1 ${commentText.trim() ? 'opacity-100' : 'opacity-50'}`}
      ...
      <BottomSheetView className="flex-1">
        <View className="flex-row items-center justify-center py-4 px-5 border-b border-gray-800">
          {currentPage === 'comments' ? (
            <Text className="text-base font-bold text-gray-100">Comments ({commentsCount})</Text>
          ) : (
            <>
              <TouchableOpacity
        ...
        {currentPage === 'comments' ? (
          ...
          >
            {data?.data?.comments && data.data.comments.length > 0 ? (
              data.data.comments.map((comment) => renderComment({ item: comment }))
            ) : (
          ...
          >
            ...
            {replyToComment?.replies && replyToComment.replies.length > 0 && (
              <View className="mt-4">
                <Text className="text-sm font-bold text-gray-500 mb-3 ml-3">Replies</Text>
                {replyToComment.replies.map((reply) => renderComment({ item: reply }))}
              </View>
            )}
          </BottomSheetScrollView>
        )}
