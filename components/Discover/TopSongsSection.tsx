...
const TopSongsSection = ({ backgroundImage, title, type = "location" }) => {
  ...
  const transformSongData = (songData) => {
    ...
    return songData.map((song) => ({
      ...
      image:
        song?.release?.artwork?.high || "https://placeholder-image-url.jpg",
      stats: {
  ...
  const calculateSongScore = (song) => {
    ...
    const maxPlaylists = Math.max(
      ...songs.map((s) => s.stats.playlistAdditions)
    );
    ...
    const playlistScore = maxPlaylists
      ? song.stats.playlistAdditions / maxPlaylists
      : 0;
  ...
  useEffect(() => {
    const fetchSongs = async () => {
      try {
        ...
        const combinedSongs = [
          ...transformSongData(locationSongs),
          ...transformSongData(worldwideSongs),
        ];
  ...
  return (
    <View className=" h-[653px] mt-[31px] mx-auto pl-[32px] pt-[32px] gap-y-[24px] rounded-[24px] overflow-hidden">
      <View className="flex-row items-center w-full justify-between pr-[29px]">
        ...
        <View className="flex-row items-center">
          <Text className="text-[14px] font-PlusJakartaSansBold text-[#FFF8F4]">
            See all
          </Text>
