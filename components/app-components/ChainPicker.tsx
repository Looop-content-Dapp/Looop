...
interface ChainPickerProps {
  onChainSelect?: (chain: { type: string; walletAddress: string }) => void;
  userWallets?: {
    xion?: string;
    starknet?: string;
  };
...
const ChainPicker: React.FC<ChainPickerProps> = ({ onChainSelect, userWallets }) => {
    ...
    const handleNetworkChange = (network: string) => {
      ...
      if (onChainSelect) {
        const chainData = {
          ...
          walletAddress: network === 'Xion' 
            ? userWallets?.xion || ''
            : userWallets?.starknet || ''
        };
        onChainSelect(chainData);
      }
    };
      ...
      <View className="bg-[#0A0B0F] border border-[#1B1E26] px-[8px] py-[11px] rounded-[16px]">
        <View className="flex-row items-center bg-[#12141B] justify-between rounded-[14px] pl-[21px] pr-[12px] py-[11px]">
          ...
          <FilterButton
            ...
            onOptionSelect={handleNetworkChange}
            icon={networkIcon}
          />
        </View>
